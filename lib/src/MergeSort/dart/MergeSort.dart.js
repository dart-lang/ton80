...
["benchmark_harness", "package:benchmark_harness/benchmark_harness.dart", , Z, {
  ...
  BenchmarkBase: {
    ...
    measure$0: function() {
      ...
      var result = Z.BenchmarkBase_measureFor(new Z.BenchmarkBase_measure_closure0(this), 2000);
      this.teardown$0();
      return result;
    }
  },
...
["dart.core", "dart:core", , P, {
  ...
  List_List$from: function(other, growable) {
    var list, t1;
    list = [];
    for (t1 = new H.ListIterator(other, other.length, 0, null); t1.moveNext$0();)
      list.push(t1._current);
    if (growable)
      return list;
...
["", "mergesortbench.dart", , B, {
  "^": "",
  main: function() {
    var line = "Template(RunTime): " + H.S(new B.TemplateBenchmark("Template", C.C_PrintEmitter).measure$0()) + " us.";
    H.printString(line);
  },
  createRandomNumbers: function(count, max) {
    var randomNumbers, i;
    randomNumbers = [];
    for (i = 0; i < count; ++i)
      randomNumbers.push(C.C__JSRandom.nextInt$1(max));
    return randomNumbers;
  },
  mergeSort: function(toSort) {
    var t1, center;
    t1 = toSort.length;
    if (t1 <= 1)
      return P.List_List$from(toSort, true);
    center = C.JSInt_methods._tdivFast$1(t1, 2);
    return B.merge(B.mergeSort(J.getInterceptor$a(toSort).sublist$2(toSort, 0, center)), B.mergeSort(C.JSArray_methods.sublist$2(toSort, center, toSort.length)));
  },
  merge: function(sortedLeft, sortedRight) {
    var result, t1, t2;
    result = [];
    while (true) {
      t1 = sortedLeft.length;
      t2 = t1 > 0;
      if (!(t2 && sortedRight.length > 0))
        break;
      if (0 >= t1)
        return H.ioore(sortedLeft, 0);
      t1 = sortedLeft[0];
      if (0 >= sortedRight.length)
        return H.ioore(sortedRight, 0);
      t2 = sortedRight[0];
      if (typeof t1 !== "number")
        return t1.$le();
      if (typeof t2 !== "number")
      ...
      if (t1 <= t2) {
        result.push(t1);
        C.JSArray_methods.removeAt$1(sortedLeft, 0);
      } else {
        result.push(t2);
        C.JSArray_methods.removeAt$1(sortedRight, 0);
      }
    }
    if (t2)
      C.JSArray_methods.addAll$1(result, sortedLeft);
    if (sortedRight.length > 0)
      C.JSArray_methods.addAll$1(result, sortedRight);
    return result;
  },
  TemplateBenchmark: {
    "^": "BenchmarkBase;name,emitter",
    run$0: function() {
      B.mergeSort($.TemplateBenchmark_toSort);
    },
    setup$0: function() {
      $.TemplateBenchmark_toSort = B.createRandomNumbers(200, 10000);
    },
    teardown$0: function() {
    },
    static: {"^": "TemplateBenchmark_toSort"}
  }
},
1],
]);
...
C.C_PrintEmitter = new Z.PrintEmitter();
