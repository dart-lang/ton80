...
//
class CFG {
  final Map<int, BasicBlock> basicBlockMap = new HashMap();
...
class SimpleLoop {
  ...
  int checksum() {
    ...
    basicBlocks.forEach((e) => result = mix(result, e.name));
    children.forEach((e) => result = mix(result, e.checksum()));
    return result;
  }
}
...
class LSG {
  ...
  int checksum() {
    ...
    loops.forEach((e) => result = mix(result, e.checksum()));
